...
gdf_error construct_join_output_df(
  ...
  PUSH_RANGE("LIBGDF_JOIN_OUTPUT", JOIN_COLOR);
    ...
    for (int i = 0; i < left_table_end; ++i) {
        ...
        RMM_TRY( RMM_ALLOC((void**)&(result_cols[i]->valid), sizeof(gdf_valid_type)*PaddedLength(get_number_of_bytes_for_valid(join_size)), 0) );
        CUDA_TRY( cudaMemset(result_cols[i]->valid, 0, sizeof(gdf_valid_type)*PaddedLength(get_number_of_bytes_for_valid(join_size))) );
    }
