...
#include <utilities/index_range.hpp>
#include <utilities/fill.cuh>
#include <utilities/constexpr_equal_range.cuh>
...
)
{
    ...
    auto warp_validity_bit_container = __ballot(lane_validity_bit);
    ...
    if (am_first_lane_in_warp) {
        ...
        auto bit_container_pos = gdf::util::detail::bit_container_index<uint32_t, gdf_size_type>(thread_needle_index);
...
// TODO:
// 1. Double-check we really have no use for the __restrict__'s here
// 2. What about constness?
gdf_error validate_inputs(
    gdf_column *           result,
...
)
{
    ...
    GDF_REQUIRE(is_nullable(*result) or use_haystack_length_for_not_found, GDF_VALIDITY_MISSING);
...
{
    GDF_TRY(detail::validate_inputs(results, sorted_haystack, needles, num_columns, use_haystack_length_for_not_found));
    ...
    // TODO: Sort the needles, up to the warp level, so that each warp gets to act on warps that are relatively close together.
    ...
    auto grid_config { cudf::util::form_naive_1d_grid(num_needles, threads_per_block) };
