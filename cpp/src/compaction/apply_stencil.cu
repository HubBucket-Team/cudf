...
enum : unsigned { bits_per_container = cudf::util::size_in_bits<bit_container>() };
...
template <bool Inclusivity>
__fd__ unsigned prefix_population_count(lane_mask_t mask)
{
...
template <bool Inclusivity>
__fd__ unsigned bit_prefix_sum(int cond)
{
    return prefix_population_count<Inclusivity>(builtins::warp_ballot(cond));
}


template <typename T, unsigned NumWarpsPerBlock, bool Inclusivity>
__fd__ T prefix_sum(T x)
{
    // TODO: Don't use the wasteful cub here - shuffling should be better. Also,
    // it assumes all threads must obtain the result, which is an assumptions
    // we don't necessarily make.
    //
    ...
    using warp_reduce_faux_class = cub::WarpScan<T>;
    __shared__ typename cub::WarpReduce<T>::TempStorage reduction_scratch_space[NumWarpsPerBlock];

    return Inclusivity ?
        warp_reduce_faux_class{reduction_scratch_space}.InclusiveSum(x) :
        warp_reduce_faux_class{reduction_scratch_space}.ExclusiveSum(x);
}
...
)
{
    ...
    at_warp_stride(
        ...
        [&](Size unpacked_pos) {
            ...
            auto output_pos = warp_base_output_pos + ::warp::prefix_population_count<exclusive>(warp_pass_mask);
