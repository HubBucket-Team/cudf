...
/* --------------------------------------------------------------------------*
 * @brief Gathers string data according indices in output_column from the category in input_column
 *
 * This function is a binary function. It will take in two gdf_columns.

 * The first one is expected to be a GDF_STRING_CATEGORY gdf_column, the second one
 * has to be a column of the same type as the first.
 * 
 * This function is used when the output_column was generated by one of our algorithms in cudf
 * that was able to operate on the indices alone. We need to take the NVCategory inside of the
 * that the output_column's indices originate from and create a new NVCategory for our output_column. 
 * Instead of just copying the original NVCategory we compact it to remove unused strings.
 * 
 * @Param[in] input_column A gdf_column from whose nvcategory we wish to gather strings
 * @Param[in,out] output_column A gdf_column that contains indices that map to the nvcategory inside of input_column
 * 
 * @Returns GDF_SUCCESS upon successful completion
 *
 * --------------------------------------------------------------------------*/
