...
BM_CPU_LoopReplace(benchmark::State &state) {
  ...
  for (auto _ : state) {
    for (std::size_t i = 0; i < vector.size(); i++) {
      auto current = std::find(to_replace_vector.begin(), to_replace_vector.end(), vector[i]);
      if (current != to_replace_vector.end()) {
        std::size_t j = std::distance(to_replace_vector.begin(), current);
...
BM_GPU_LoopReplace(benchmark::State &state) {
  ...
  std::vector<T> device_vector(length);
  thrust::sequence(device_vector.begin(), device_vector.end(), 1);
  gdf_col_pointer column = create_gdf_column(device_vector);

  std::vector<T> to_replace_vector(10);
  thrust::sequence(to_replace_vector.begin(), to_replace_vector.end(), 1);
  gdf_col_pointer to_replace = create_gdf_column(to_replace_vector);

  std::vector<T> values_vector(10);
  thrust::sequence(values_vector.begin(), values_vector.end(), 1);
  gdf_col_pointer values = create_gdf_column(values_vector);

  for (auto _ : state) {
    gdf_error status = gdf_find_and_replace_all(column.get(), to_replace.get(), values.get());
